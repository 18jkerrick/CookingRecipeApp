...
export default function RecipeDetailModal({ isOpen, onClose, recipe, isSaved = false, onSave, onDelete, onUpdate }: RecipeDetailModalProps) {
  ...
  const addMealToPlan = (dayIndex: number, mealType: string) => {
    ...
    if (recipe) {
      ...
      addRecipeToMealPlan(dateKey, mealType, recipe);
                    ...
                    <button
                      onClick={async () => {
                        ...
                        const newList = await createGroceryList(newGroceryListName, [{
                          ...
                          original_url: '',
                          created_at: new Date().toISOString(),
                          saved_id: recipe.saved_id || Date.now().toString()
                        }]);
                        ...
                        {Object.entries(dayMeals).map(([mealType, meal]: [string, any]) => (
                          <div key={mealType} className="flex items-center space-x-3 p-3 bg-white/5 rounded-lg">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-sm capitalize" style={{color: MEAL_TYPES.find(m => m.type === mealType)?.color || '#FF3A25'}}>{mealType}</p>
                            </div>
                            <button
             ...
             <div className="p-6 space-y-4">
               ...
               >
                ...
                >
                  ...
                  <span className="text-lg font-medium text-white">Morning Snack</span>
                </button>
                ...
                >
                  ...
                  <span className="text-lg font-medium text-white">Afternoon Snack</span>
                </button>
