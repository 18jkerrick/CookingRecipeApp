...
function extractFromCaptions(text: string): string | null {
  ...
  // Pattern 0: Look for clear recipe titles at the very beginning (before cleaning removes serving info)
  ...
  // Pattern 1: Look for recipe titles after common Facebook/social media phrases
  ...
  // Pattern 2: Look for the first sentence that contains food words (but skip instruction sentences)
  const sentences = text.split(/[.!?]/).map(s => s.trim());
  
  for (const sentence of sentences) {
    if (sentence.length < 5 || sentence.length > 100) continue;
    
    // Skip introductory sentences and instruction sentences
    if (/^(Basics Done Right|Because|Follow|Subscribe|Like|Comment|Share|In a|In the|Add|Mix|Whisk|Combine|Stir|Beat|Fold|Pour|Bake|Cook|Heat|Preheat)/i.test(sentence)) {
      continue;
    }
    
    // Validate it looks like a food title
    if (isValidFoodTitle(sentence)) {
      return cleanTitle(sentence);
    }
  }
  
  // Pattern 3: Fallback to first sentence if it's reasonable
  ...
  if (firstSentenceMatch) {
    ...
    // Only use if it contains food words and isn't too generic or an instruction
    if (isValidFoodTitle(candidate) && 
        !/(follow|subscribe|like|comment|share|bio|link|in a|in the|add|mix|whisk|combine)/i.test(candidate)) {
